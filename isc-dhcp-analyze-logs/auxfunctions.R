...
# Nearly like matlabcolors - but better I think
...
# We try different X thresholds and report precision and power
...
plotHistFactor <- function(X, f, breaks) {
  B <- breaks
  f <- as.factor(f)
  levs <- levels(f)
  M <- NULL
...
rlcMultiHist <- function(X, C, breaks="Sturges", col=NULL, main=NULL, xlab=NULL, add.legend=TRUE, xlim=range(breaks)) {
  ...
  if (add.legend) {
    legend(x="topright", legend=rev(cls), col=rev(col), pch=15)
  }
}
# Example:
...
# rlcMultiDensOLD <- function(X, C, bw="nrd0", kernel="gaussian", col=NULL, main=NULL, xlab=NULL, ylab="Density", add.legend=TRUE, xlim=NULL) {
#   if (length(X) != length(C)) stop("X and C must have the same length")
#   cls <- sort(unique(C))
#   xdesc <- deparse(substitute(X))
#   if (is.null(xlim)) {
#     h <- density(X, bw=bw, kernel=kernel)
#   } else {
#     h <- density(X, bw=bw, kernel=kernel, from=xlim[1], to=xlim[2])
#   }
#   M <- matrix(0, nrow=length(cls), ncol=length(h$x))
#   i <- 1
#   for (cl in cls) {
#     hc <- density(X[C==cl], bw=h$bw, kernel=kernel, from=h$x[1], to=h$x[length(h$x)])
...
#     xlab <- xdesc
#   }
#   plot(h$x, h$y, main=mainTitle, xlab=xlab, ylab=ylab, ylim=c(0, max(h$y)), type="l")
#   
#   if (is.null(col)) {
#     col <- mapColors(1:nrow(M))
#   }
...
#   
#   if (add.legend) {
#     legend(x="topright", legend=cls, col=col, pch=15)
#   }
# }
# Example:
#rlcMultiDensOLD(iris$Petal.Length, iris$Species)



# Show cumulative density colored by class.
rlcMultiDens <- function(X, C, bw="nrd0", kernel="gaussian", weights=NULL, col=NULL, main=NULL, xlab=NULL, ylab="Density", add.legend=TRUE, xlim=NULL, showProgress=FALSE) {
  if (showProgress) {
    progressBarInit()
  }
  if (length(X) != length(C)) stop("X and C must have the same length")
...
# 
# rlcCompareDensOLD <- function(X, C, bw="nrd0", kernel="gaussian", col=NULL, main=NULL, xlab=NULL, ylab="Density", add.legend=TRUE, xlim=NULL) {
#   if (length(X) != length(C)) stop("X and C must have the same length")
#   cls <- sort(unique(C))
#   xdesc <- deparse(substitute(X))
#   if (is.null(xlim)) {
...
#     xlab <- xdesc
#   }
#   plot(h$x, h$y, main=mainTitle, xlab=xlab, ylab=ylab, ylim=c(0, ymax), type="n")
#   
#   if (is.null(col)) {
#     col <- mapColors(1:nrow(M))
#   }
...
#   X <- h$x
#   for (i in nrow(M):1) {
#     polygon(c(X[1],X,X[length(X)]), c(0,M[i,],0), col=NA, border=col[[i]])
#   }
#   
#   if (add.legend) {
#     legend(x="topright", legend=cls, col=col, pch=15)
...
# }
# 
# rlcCompareDensOLD(c(1,1,10,10,10,10,10,10), c(1,1,2,2,2,2,2,2))



rlcCompareDens <- function(X, C, bw="nrd0", kernel="gaussian", weights=NULL, col=NULL, main=NULL, xlab=NULL, ylab="Density", add.legend=TRUE, xlim=NULL, showProgress=FALSE, ylim=NULL) {
  if (showProgress) {
    progressBarInit()
  }
  if (length(X) != length(C)) stop("X and C must have the same length")
  ...
  plot(h$x, h$y, main=mainTitle, xlab=xlab, ylab=ylab, ylim=ylim, type="n")
...
# but more likes measures of a real function at
# some points X.
...
rlcPlotManyFromMatrix <- function(M, lineColors=NULL) {
  if (ncol(M) < 2) {
    stop("Matrix must have at least 2 columns (for X and Y1)")
  }
  if (is.null(lineColors)) {
    lineColors <- mapColors(1:ncol(M), col=clearcolors)
  }
  ymin <- min(M[,2:ncol(M)])
  ymax <- max(M[,2:ncol(M)])
  plot(M[,1], M[,2], type="l", col=clearcolors[[1]], xlab=NA, ylab=NA, ylim=c(ymin,ymax))
  if (ncol(M) > 2) {
    for (j in 3:ncol(M)) {
      lines(M[,1], M[,j], col=lineColors[[j]])
    }
  }
}
...
makeHistGeneralAverageNew <- function(X, Y, breaks, verbose=FALSE) {
  ...
  return(list(breaks=breaks, counts=Ysum, mids=makeHistMids(breaks)))
}
...
rlcNearlyEqual <- function(X, Y, relativeError=1E-5) {
  return(abs(X - Y)/abs(pmax(X, Y)) < relativeError)
}
#X <- c(40, 1E10, 20)
...
rlcHeatMatrix <- function(X, Y, xbreaks=10, ybreaks=10, col=defaultcolors) {
  xdesc <- deparse(substitute(X))
  ydesc <- deparse(substitute(Y))
  ...
  image(hx$breaks, hy$breaks, M, col=col, xlab=xdesc, ylab=ydesc, zlim=r)
