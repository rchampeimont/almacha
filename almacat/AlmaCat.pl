...
sub speaks {
  ...
  if ($text =~ m/^$myname help/i) {
    ...
    mm("Je suis un bot mis dans le domaine public.\nCommandes internes : quiet talk kick hop unicode utf rand\nCommandes externes : ($myname exec commande) :\n@cmdlist");
    return;
  }

  # rand
  if ($text =~ m/^$myname rand/i) {
    mm(rand());
    return;
  }

  # unicode
  if ($text =~ m/^$myname unicode/i) {
    my @args = split(/ /, $text);
    my $charnum = $args[2];
    if (defined $charnum and $charnum ne "") {
      mm("U+$charnum " . charnames::viacode(hex($charnum)) . "   UTF-8: "
	. encode("UTF-8", chr(hex($charnum)), Encode::FB_DEFAULT));
    } else {
      mm("syntax: $myname unicode charnumber");
    }
    return;
  }

  # utf
  if ($text =~ m/^$myname utf/i) {
    my @args = split(/ /, $text);
    my $char = $args[2];
